...
std::optional<mpss::KeyInfo> mpss::impl::os::android_key_info_from_security_level(std::int32_t security_level) noexcept
{
                                    ...
                                    IsolationLevel minimum_isolation)
{
    ...
    jmethodID method = env->GetStaticMethodID(key_management, "CreateKey",
                                              "(Ljava/lang/String;Lcom/microsoft/research/mpss/Algorithm;Z)I");
    ...
    auto inspect_created_key = [&]() -> std::unique_ptr<KeyPair> {
        ...
        {
            mpss::utils::log_and_set_error(
                "Newly created Android key '{}' measured at isolation level {} below the requested minimum isolation "
                "{}.",
                name, static_cast<unsigned>(key_info->isolation_level), static_cast<unsigned>(minimum_isolation));
    ...
    const AndroidCreateOutcome strongbox_outcome = create_in_store(/* use_strongbox */ true);
    ...
    if (IsolationLevel::hardware != minimum_isolation && AndroidCreateOutcome::unavailable == strongbox_outcome)
    {
