...
{
    return os::create_key(name, algorithm, policy, min_security);
}

std::unique_ptr<KeyPair> OSBackend::open_key(std::string_view name, SecurityType min_security) const
{
    return os::open_key(name, min_security);
}
