...
bool is_algorithm_available(Algorithm algorithm, IsolationLevel minimum_isolation)
{
    ...
    // Cache successful probes by algorithm and minimum isolation.
    constexpr std::size_t isolation_level_count = static_cast<std::size_t>(IsolationLevel::hardware) + 1;
    static std::mutex cache_mutex;
    static std::array<std::array<bool, isolation_level_count>, algorithm_info.size()> cache{};
...

    {
        ...
        if (cache[algorithm_index][isolation_index])
        {
    ...
    {
        std::scoped_lock lock{cache_mutex};
        cache[algorithm_index][isolation_index] = true;
    }
...
KeyPair::KeyPair(Algorithm algorithm, bool hardware_backed, const char *storage_description)
    : KeyPair{algorithm, KeyInfo{hardware_backed, storage_description}}
{
}

KeyPair::KeyPair(Algorithm algorithm, IsolationLevel isolation_level, const char *storage_description)
    : KeyPair{algorithm, KeyInfo{isolation_level, storage_description}}
{
}

KeyPair::KeyPair(Algorithm algorithm, KeyInfo key_info)
    : algorithm_{algorithm}, info_{get_algorithm_info(algorithm)}, key_info_{key_info}
{
    if (0 == info_.key_bits)
    {
