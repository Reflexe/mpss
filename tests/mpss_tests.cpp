...
SECURITY_STATUS RawCreatePersistedKey(LPCWSTR provider_name, const std::wstring &key_name, DWORD create_flags)
{
    ...
    SECURITY_STATUS status = ::NCryptOpenStorageProvider(&provider, provider_name, 0);
    ...
    status = ::NCryptCreatePersistedKey(provider, &key, NCRYPT_ECDSA_P256_ALGORITHM, key_name.c_str(), 0, create_flags);
    if (ERROR_SUCCESS != status)
    {
    ...
    status = ::NCryptSetProperty(key, NCRYPT_EXPORT_POLICY_PROPERTY, reinterpret_cast<PBYTE>(&export_policy),
    ...
    else if (ERROR_SUCCESS != ::NCryptDeleteKey(key, 0))
    {
        ::NCryptFreeObject(key);
    }
    ::NCryptFreeObject(provider);
