                                   ...
                                   [[maybe_unused]] void *pw_cbarg)
{
    ...
    // The object callback consumes the reference synchronously, so the unique_ptr can own it and
    // free it on return.
    mpss_key_reference *reference_ptr = key_reference.get();
    int object_type = OSSL_OBJECT_PKEY;
    OSSL_PARAM params[4];
    params[0] = OSSL_PARAM_construct_int(OSSL_OBJECT_PARAM_TYPE, &object_type);
    params[1] = OSSL_PARAM_construct_utf8_string(OSSL_OBJECT_PARAM_DATA_TYPE,
                                                 const_cast<char *>(mpss_reference_object_data_type), 0);
    params[2] = OSSL_PARAM_construct_octet_string(OSSL_OBJECT_PARAM_REFERENCE, &reference_ptr, sizeof(reference_ptr));
