...
extern "C" int mpss_digest_init(void *ctx, [[maybe_unused]] const OSSL_PARAM params[])
{
    ...
    if (dctx->state != uninitialized && dctx->state != error)
    {
        dctx->state = uninitialized;
    ...
    else
    {
        ...
        // Free any leftover resources from a previous error state before reallocating.
        if (nullptr != dctx->evp_dctx)
        {
            EVP_MD_CTX_free(dctx->evp_dctx);
            dctx->evp_dctx = nullptr;
        }
        if (nullptr != dctx->evp_md)
        {
            EVP_MD_free(dctx->evp_md);
            dctx->evp_md = nullptr;
        }
...
extern "C" int mpss_digest_update(void *ctx, const unsigned char *in, ::size_t inl)
{
    ...
    if (nullptr == dctx || (dctx->state != digesting))
    {
        return 0;
    }
...
extern "C" int mpss_digest_final(void *ctx, unsigned char *out, ::size_t *outl, ::size_t outsz)
{
    ...
    // Check that the digest context is available and in the right state.
    // This means that the EVP_MD_CTX and EVP_MD are already set up.
    const digest_state state = dctx->state;
...
#define MPSS_MAKE_DIGEST_DISPATCH_TABLE(digest, digestsz, blocksz)                                                     \
    const OSSL_DISPATCH mpss_digest_functions_##digest[] = {                                                           \
        ...
        {OSSL_FUNC_DIGEST_DIGEST, reinterpret_cast<void (*)(void)>(mpss_digest_digest_##digest)},                      \
        {OSSL_FUNC_DIGEST_GETTABLE_PARAMS, reinterpret_cast<void (*)(void)>(mpss_digest_gettable_params)},             \
        {OSSL_FUNC_DIGEST_GET_PARAMS, reinterpret_cast<void (*)(void)>(mpss_digest_get_params_##digest)},              \
        OSSL_DISPATCH_END};
